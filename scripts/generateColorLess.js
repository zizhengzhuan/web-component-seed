...
if (fs.existsSync(entry)) {
  content = fs.readFileSync(entry).toString();
}
...
content += `@import "${path.join(themePath, 'theme/static/index.less')}";\n`;
...
less.render.call(less, content, {
  ...
  const bezierEasing = fs.readFileSync(path.join(themePath, 'theme/static/style/color/bezierEasing.less')).toString();
  const tinyColor = fs.readFileSync(path.join(themePath, 'theme/static/style/color/tinyColor.less')).toString();
  const colorPalette = fs.readFileSync(path.join(themePath, 'theme/static/style/color/colorPalette.less'))
    .toString()
